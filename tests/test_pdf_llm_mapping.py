...
def test_llm_nested_response_maps_to_pdf_side_data():
    raw = PdfExtractionLLMResponse(
        ...
        totals=PdfTotalsExtract(total_value=99.9, liquid_value=88.0),
    )
    ...
    assert side.liquid_value == 88.0
    assert side.taxes_note == "ICMS destacado"
    assert side.iss is None


def test_merge_prefers_llm_invoice_number_date_totals_when_used_llm():
    det = PdfSideData(
        issuer=Party(cnpj="1"),
        invoice_number="84051900",
        date="10/10/2025",
        total_value=100.0,
        liquid_value=50.0,
        used_llm=False,
    ...
    llm = PdfSideData(
        issuer=Party(name="X"),
        invoice_number="46466",
        date="2025-10-10",
        total_value=6389.68,
        liquid_value=6092.56,
        used_llm=True,
        extraction_mode="llm",
    )
    merged = _merge_det_llm(det, llm)
    assert merged.invoice_number == "46466"
    assert merged.date == "2025-10-10"
    assert merged.total_value == 6389.68
    assert merged.liquid_value == 6092.56
